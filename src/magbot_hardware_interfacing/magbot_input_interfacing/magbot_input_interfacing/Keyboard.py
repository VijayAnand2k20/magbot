...
class Keyboard(Node):
    ...
    def publish_current_command(self):
        ...
        self.current_joy_message.header.stamp = self.get_clock().now()
    ...
    def run(self):
        ...
        if os.getenv("DISPLY", default="-") != "-":
            self.get_logger().fatal("This device does not have a display connected. The keyboard node requires a connected display due to a limitation of the underlying package. Keyboard node now shutting down")
            # rospy.sleep(1)
            self.rate.sleep()
            sys.exit(0)
