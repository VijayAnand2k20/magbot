...
if len(sys.argv) > 1 and sys.argv[1] in servo_dict:
    ...
    if servo_name != "relax":
        print('Magbot: Motor ' + servo_name + ' moved to ' + servo_move + '.\n')
    else:
...
if servo_name == "fl" or servo_name == "all":
    Magbot.moveAbsAngle(Magbot.front_left_hip   ,offsets[0,1]+pos[0])
    Magbot.moveAbsAngle(Magbot.front_left_upper ,offsets[1,1]+pos[1])
...
if servo_name == "bl" or servo_name == "all":
    Magbot.moveAbsAngle(Magbot.back_left_hip    ,offsets[0,3]+pos[0])
    Magbot.moveAbsAngle(Magbot.back_left_upper  ,offsets[1,3]+pos[1])
    Magbot.moveAbsAngle(Magbot.back_left_lower  ,offsets[2,3]+pos[2])

if servo_name == "relax":
    Magbot.relax_all_motors()
